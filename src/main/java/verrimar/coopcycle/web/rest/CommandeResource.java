...
import verrimar.coopcycle.service.CommandeService;
import verrimar.coopcycle.service.dto.CommandeDTO;
import verrimar.coopcycle.web.rest.errors.BadRequestAlertException;

/**
 * REST controller for managing {@link verrimar.coopcycle.domain.Commande}.
 */
@RestController
@RequestMapping("/api")
public class CommandeResource {
    ...
    public List<CommandeDTO> getAllCommandes(@RequestParam(required = false) String filter) {
        if ("paiement-is-null".equals(filter)) {
            log.debug("REST request to get all Commandes where paiement is null");
            return commandeService.findAllWherePaiementIsNull();
        }
