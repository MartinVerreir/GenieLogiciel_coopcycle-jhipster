...
import verrimar.coopcycle.service.PaiementService;
import verrimar.coopcycle.service.dto.PaiementDTO;
import verrimar.coopcycle.web.rest.errors.BadRequestAlertException;

/**
 * REST controller for managing {@link verrimar.coopcycle.domain.Paiement}.
 */
@RestController
@RequestMapping("/api")
public class PaiementResource {
    ...
    public List<PaiementDTO> getAllPaiements(@RequestParam(required = false) String filter) {
        if ("panier-is-null".equals(filter)) {
            log.debug("REST request to get all Paiements where panier is null");
            return paiementService.findAllWherePanierIsNull();
        }
