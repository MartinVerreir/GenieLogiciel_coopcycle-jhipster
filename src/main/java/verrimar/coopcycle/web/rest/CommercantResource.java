...
import verrimar.coopcycle.service.CommercantService;
import verrimar.coopcycle.service.dto.CommercantDTO;
import verrimar.coopcycle.web.rest.errors.BadRequestAlertException;

/**
 * REST controller for managing {@link verrimar.coopcycle.domain.Commercant}.
 */
@RestController
@RequestMapping("/api")
public class CommercantResource {
    ...
    /**
     * {@code SEARCH  /_search/commercants?query=:query} : search for the commercant corresponding
     * to the query.
     *
     * @param query the query of the commercant search.
     * @return the result of the search.
     */
    @GetMapping("/_search/commercants")
    public List<CommercantDTO> searchCommercants(@RequestParam String query) {
        log.debug("REST request to search Commercants for query {}", query);
        return commercantService.search(query);
    }
}
