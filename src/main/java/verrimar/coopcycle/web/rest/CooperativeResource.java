...
import verrimar.coopcycle.service.CooperativeService;
import verrimar.coopcycle.service.dto.CooperativeDTO;
import verrimar.coopcycle.web.rest.errors.BadRequestAlertException;

/**
 * REST controller for managing {@link verrimar.coopcycle.domain.Cooperative}.
 */
@RestController
@RequestMapping("/api")
public class CooperativeResource {
    ...
    /**
     * {@code SEARCH  /_search/cooperatives?query=:query} : search for the cooperative corresponding
     * to the query.
     *
     * @param query the query of the cooperative search.
     * @return the result of the search.
     */
    @GetMapping("/_search/cooperatives")
    public List<CooperativeDTO> searchCooperatives(@RequestParam String query) {
        log.debug("REST request to search Cooperatives for query {}", query);
        return cooperativeService.search(query);
    }
}
