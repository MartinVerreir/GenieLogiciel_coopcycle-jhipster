...
import verrimar.coopcycle.service.PanierService;
import verrimar.coopcycle.service.dto.PanierDTO;
import verrimar.coopcycle.web.rest.errors.BadRequestAlertException;

/**
 * REST controller for managing {@link verrimar.coopcycle.domain.Panier}.
 */
@RestController
@RequestMapping("/api")
public class PanierResource {
    ...
    /**
     * {@code SEARCH  /_search/paniers?query=:query} : search for the panier corresponding
     * to the query.
     *
    ...
    public List<PanierDTO> searchPaniers(@RequestParam String query) {
        log.debug("REST request to search Paniers for query {}", query);
        return panierService.search(query);
    }
}
